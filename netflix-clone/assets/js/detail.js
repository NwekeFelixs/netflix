...
const filterVideos = function(videoList)  {
    return videoList.filter(({type, site}) => (type.toLowerCase() === "trailer" || type.toLowerCase() === "teaser") && site.toLowerCase() === "youtube");
};
...
fetchDataFromServer(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${api_key}&append_to_response=casts,videos,images,releases`, function (movie){
    ...
    if (sliderInner) {
        ...
        for (let i = 0; i < filteredVideos.length; i++) {
            const { key, name, type, site } = filteredVideos[i];

            const videoCard = document.createElement("div");
            videoCard.classList.add("video-card");

            videoCard.innerHTML = `
                <iframe src="https://www.youtube.com/embed/${key}?&theme=dark&color=white&rel=0" frameborder="0" width="500" height="294" allowfullscreen="1" title="${name}" class="img-cover"></iframe>
            `;

            sliderInner.appendChild(videoCard);
        }
    } else {
        console.error("Slider Inner not found!");
    }
